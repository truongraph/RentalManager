...
class LoginPage(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        try:
            ...
            logo = ctk.CTkImage(light_image=logo_img, dark_image=logo_img, size=(130, 40))
            ctk.CTkLabel(container, image=logo, text="").pack(pady=(50, 10))
        except:
            ctk.CTkLabel(container, text="LOGO", font=("Inter", 50)).pack(pady=(50, 10))

        ctk.CTkLabel(container, text="Đăng nhập hệ thống", font=("Inter", 22, "bold"), text_color="#2756B5").pack(pady=(20,0))
        ctk.CTkLabel(container, text="Nhập tên tài khoản và mật khẩu", font=("Inter", 13), text_color="#9b9b9b").pack(pady=(0,30))
        ...
        self.entry_user = ctk.CTkEntry(form, height=50, corner_radius=7, font=("Inter", 16), fg_color="#f8fafc")
        self.entry_user.pack(fill="x", pady=(0,20))
        self.entry_user.focus()

        ctk.CTkLabel(form, text="Mật khẩu", font=("Inter", 14,"bold")).pack(anchor="w", pady=(0,5))
        self.entry_pass = ctk.CTkEntry(form, show="•", height=50, corner_radius=7, font=("Inter", 16), fg_color="#f8fafc")
        self.entry_pass.pack(fill="x", pady=(0,40))

        ctk.CTkButton(form, text="Đăng nhập ngay", command=self.login, height=56, corner_radius=7,
                      font=("Inter", 16, "bold"), fg_color="#2F66C8", hover_color="#204FAD").pack(fill="x")

        ctk.CTkLabel(container, text="© 2025 Nhóm 8", font=("Inter", 12), text_color="#000").pack(side="bottom", pady=30)
