...
class Dashboard(ctk.CTk):
    ...
    def _create_header(self):
        header = ctk.CTkFrame(self, height=100, corner_radius=0, fg_color="#001961")
    ...
    def _create_menu(self):
        ...
        menu = ctk.CTkFrame(self, height=50, corner_radius=0, fg_color="#003FD4")
                ...
                for c in w.winfo_children():
                    if isinstance(c, ctk.CTkFrame):
                        for b in c.winfo_children():
                            if (
                                isinstance(b, ctk.CTkButton)
                                and hasattr(b, "cmd")
                                and b.cmd == cmd
                            ):
                                self.active_button = b
                                b.configure(fg_color="#0068F7")
                                break
        cmd()
    ...
    def _create_footer(self):
        footer = ctk.CTkFrame(self, height=70, corner_radius=0, fg_color="#FFFFFF")
        footer.grid(row=3, column=0, sticky="ew")
        footer.pack_propagate(False)
        ctk.CTkLabel(
            footer,
            text="© 2025 Nhóm 8 • Võ Đang Trường - Đỗ Ngọc Minh Tiến - Trần Ngọc Thái Bình - Lê Tấn Thành",
            font=("Inter", 13, "bold"),
            text_color="#0067F7",
        ).pack(pady=22)
    ...
    def _load_home(self):
        self._clear()
        HomeTab(self.content, self.user_id).pack(
            fill="both", expand=True, padx=30, pady=30
        )

    def _load_room(self):
        self._clear()
        RoomTab(self.content, self.user_id).pack(
            fill="both", expand=True, padx=30, pady=30
        )

    def _load_tenant(self):
        self._clear()
        TenantTab(self.content, self.user_id).pack(
            fill="both", expand=True, padx=30, pady=30
        )

    def _load_contract(self):
        self._clear()
        ContractTab(self.content, self.user_id).pack(
            fill="both", expand=True, padx=30, pady=30
        )

    def _load_bill(self):
        self._clear()
        BillTab(self.content, self.user_id).pack(
            fill="both", expand=True, padx=30, pady=30
        )

    def _load_report(self):
        self._clear()
        ReportTab(self.content, self.user_id).pack(
            fill="both", expand=True, padx=30, pady=30
        )
