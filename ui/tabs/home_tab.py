...
class HomeTab(ctk.CTkFrame):
    ...
    def _create_stats(self):
        ...
        stats = [
            ("Tổng phòng đang có", room["total"], "#0067F7"),
            ("Đang được thuê", room["occupied"], "#00B63E"),
            ("Tổng khách hàng", tenant["active"], "#8b5cf6"),
            (
                "Hợp đồng đang thuê",
                contract["new_this_month"] + contract["soon_expire"],
                "#f59e0b",
            ),
            ("Hợp đồng sắp hết hạn", contract["soon_expire"], "#F50002"),
            ("Hóa đơn chưa thu", bill["unpaid"], "#C91D1D"),
        ]
    ...
    def _stat_card(self, parent, title, value, color, row, col):
        ...
        ctk.CTkLabel(card, text=title, font=("Inter", 14), text_color="#475569").pack(
            pady=(18, 5)
        )
        ctk.CTkLabel(
            card, text=str(value), font=("Inter", 42, "bold"), text_color=color
        ).pack(pady=(0, 20))
    ...
    def _revenue_chart(self, parent):
        ...
        if not data or all(r == 0 for r in revenues):
            ...
            ax.fill_between(months, revenues, alpha=0.15, color="#00B63E")

            ax.set_title(
                "Doanh thu 6 tháng (Đã thu)",
                fontsize=13,
                fontweight="bold",
                pad=12,
                color="#1e293b",
