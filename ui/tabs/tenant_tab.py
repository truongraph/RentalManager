...
class TenantTab(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(form, text="Họ và tên *", font=("Inter", 14, "bold")).grid(
        ...
        ctk.CTkLabel(form, text="Số điện thoại", font=("Inter", 14, "bold")).grid(
        ...
        ctk.CTkLabel(form, text="CMND/CCCD", font=("Inter", 14, "bold")).grid(
        ...
        ctk.CTkLabel(form, text="Giới tính", font=("Inter", 14, "bold")).grid(
            row=0, column=3, sticky="w", pady=(0, 5), padx=(30, 0)
        )
        ...
        ctk.CTkLabel(form, text="Ngày sinh", font=("Inter", 14, "bold")).grid(
            row=2, column=0, sticky="w", pady=(20, 5)
        )
        self.cal_birth = DateEntry(
            form,
            height=40,
            corner_radius=7,
        ...
        ctk.CTkLabel(form, text="Địa chỉ thường trú", font=("Inter", 14, "bold")).grid(
        ...
        self.entry_note.grid(
            row=3, column=2, sticky="ew", columnspan=2, padx=(30, 0), pady=(0, 20)
        )
        ...
        for col, text, w in zip(columns, headers, widths):
            ...
            self.tree.column(
                col,
                width=w,
                anchor="center" if col not in ["name", "address", "note"] else "w",
            )
    ...
    def _load_data(self):
        ...
        for t in get_all_tenants():
            ...
            if t["birth"]:
                try:
                    birth = datetime.strptime(t["birth"], "%Y-%m-%d").strftime(
                        "%d/%m/%Y"
                    )
                except:
                    birth = ""
    ...
    def on_select(self, event):
        ...
        self.entry_phone.insert(0, v[3] if v[3] != "—" else "")
        self.entry_id.delete(0, "end")
        self.entry_id.insert(0, v[4] if v[4] != "—" else "")
        self.entry_address.delete(0, "end")
        self.entry_address.insert(0, v[5] if v[5] != "—" else "")
        self.entry_note.delete(0, "end")
        self.entry_note.insert(0, v[7])
