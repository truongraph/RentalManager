...
def get_room_report():
    ...
    occupied = conn.execute("SELECT COUNT(*) FROM room WHERE status = 'occupied' AND is_deleted = 0").fetchone()[0]
    available = conn.execute("SELECT COUNT(*) FROM room WHERE status = 'available' AND is_deleted = 0").fetchone()[0]
...
def get_contract_report():
    ...
    ended = conn.execute("SELECT COUNT(*) FROM contract WHERE contract_status = 'ended' AND is_deleted = 0").fetchone()[0]

    return {"new_this_month": new_this_month, "soon_expire": soon_expire, "ended": ended}


# Thống kê hóa đơn: chưa thu / đã thu / tổng
def get_bill_report():
    conn = get_db()
    unpaid = conn.execute("SELECT COUNT(*) FROM bill WHERE paid_status = 'unpaid' AND is_deleted = 0").fetchone()[0]
    paid = conn.execute("SELECT COUNT(*) FROM bill WHERE paid_status = 'paid' AND is_deleted = 0").fetchone()[0]
