...
def generate_bill_pdf(bill_data, output_dir=PDF_OUTPUT_DIR):
    ...
    styles = {
        ...
        "total_amount": ParagraphStyle(
            ...
            fontSize=13,
            alignment=TA_RIGHT,
            textColor=colors.HexColor("#C91D1D"),
        ),
    ...
    header_right = [
        ...
        Paragraph(
            f"Ngày lập: {datetime.now().strftime('%d/%m/%Y')}", styles["info_right"]
        ),
    ]
    ...
    story.append(
        Paragraph(
            f"Tiền thuê phòng: {format_currency(bill_data['room_rent_amount'])} VNĐ",
            styles["normal"],
        )
    )
    ...
    story.append(
        Paragraph(
            f"Điện: {bill_data['elec_prev']} → {bill_data['elec_current']} kWh "
            f"(dùng {elec_used} kWh × {_format_price(bill_data['electric_unit_price'])})",
            styles["normal"],
        )
    )
    story.append(
        Paragraph(
            f"     = {format_currency(elec_used * bill_data['electric_unit_price'])} VNĐ",
            styles["normal"],
        )
    )
    ...
    story.append(
        Paragraph(
            f"Nước: {bill_data['water_prev']} → {bill_data['water_current']} m³ "
            f"(dùng {water_used} m³ × {_format_price(bill_data['water_unit_price'])})",
            styles["normal"],
        )
    )
    story.append(
        Paragraph(
            f"     = {format_currency(water_used * bill_data['water_unit_price'])} VNĐ",
            styles["normal"],
        )
    )
    ...
    if bill_data.get("other_fee", 0) > 0:
        story.append(
            Paragraph(
                f"Phí khác: {format_currency(bill_data['other_fee'])} VNĐ",
                styles["normal"],
            )
        )
    ...
    story.append(
        Paragraph(
            f"TỔNG CỘNG THANH TOÁN: {format_currency(bill_data['total_amount'])} VNĐ",
            styles["total_label"],
        )
    )
    story.append(Spacer(1, 10))
    status_text = (
        "Đã thanh toán" if bill_data["paid_status"] == "paid" else "Chưa thanh toán"
    )
